...
drop table if exists payme_sandbox.p2p_user_activation_sessions_hour_of_day;
create table payme_sandbox.p2p_user_activation_sessions_hour_of_day as
...
select user_id, hour_of_day
    from
    (
    select user_id, hour_of_day, rank() over (partition by user_id order by session_count desc) rn
        from
        (
        select u._id user_id, extract(hour from j.date) as hour_of_day, count(*) session_count
