drop table if exists payme_sandbox.p2p_user_activation_sessions_day_of_week;
create table payme_sandbox.p2p_user_activation_sessions_day_of_week as
...
select user_id, day_of_week
    from
    (
    select user_id, day_of_week, rank() over (partition by user_id order by session_count desc) rn
        from
        (
        select u._id user_id, extract(dow from j.date) day_of_week, count(*) session_count
